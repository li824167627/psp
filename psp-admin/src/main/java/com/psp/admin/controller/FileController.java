...
import com.psp.admin.controller.res.bean.RImgCodeBean;
import com.psp.admin.controller.res.bean.RQiniuFileBean;
import com.psp.admin.controller.springmvc.req.getImgcodeParam;
import com.psp.admin.controller.springmvc.req.getLoginImgcodeParam;
import com.psp.admin.model.Code;
import com.psp.admin.service.QiniuService;
...
public class FileController {
	...
	public ObjectResult<RImgCodeBean> getImgCode(getImgcodeParam param, HttpServletRequest request,
		...
		try {
			...
			if (base64img != null) {
				...
				adminCacheImpl.setImgCode(imgToken, code);
				result.setData(data);
				result.setFlag(true);
			}
	...
	public BaseResult getLoginImgCode(getLoginImgcodeParam param, HttpServletRequest request,
		...
		try {
			...
			Code code = adminCacheImpl.getLoginCode(userId);
			if (code == null) {
				code = new Code();
				code.setNum(5);
			}
			code.setCode(verifyCode);
			code.setTime(System.currentTimeMillis());
			adminCacheImpl.setLoginCode(userId, code);
		} catch (MalformedURLException e) {
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
